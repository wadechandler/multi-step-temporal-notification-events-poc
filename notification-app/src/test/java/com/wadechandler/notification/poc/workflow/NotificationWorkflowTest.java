...
class NotificationWorkflowTest {
    ...
    void setUp() {
        ...
        // Workflow worker — polls NOTIFICATION_QUEUE for workflow tasks
        ...
        // Activity workers — the workflow routes activities to separate queues
        // via ActivityOptions.setTaskQueue(), so the test needs workers on those queues.
        contactActivities = mock(ContactActivities.class);
        messageActivities = mock(MessageActivities.class);

        Worker contactWorker = testEnv.newWorker(TaskQueues.CONTACT_ACTIVITY_QUEUE);
        contactWorker.registerActivitiesImplementations(contactActivities);

        Worker messageWorker = testEnv.newWorker(TaskQueues.MESSAGE_ACTIVITY_QUEUE);
